...
contains
    ...
    function domain_constructor(config) result(domain)
        ...
        domain % equilib  = distrib_t(config % num_x, config % num_y)
        domain % density  = density_t(config % num_x, config % num_y)
        domain % velocity = velocity_t(config % num_x, config % num_y)
